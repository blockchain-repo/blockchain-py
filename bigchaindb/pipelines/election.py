...
class Election:
    ...
    def check_for_quorum(self, next_vote):
        ...
        self.bigchain.save_block_status(next_block['block']['node_pubkey'], next_block['id'], block_status,
                                        len(next_block['block']['transactions']))
        if block_status == self.bigchain.BLOCK_INVALID:
            try:
                block = Block.from_dict(next_block)
                return block
            except InvalidHash:
                logger.debug("InvalidHash %s", next_block['id'])
                transactions = [Transaction.from_dict(tx) for tx
                                in next_block['block']['transactions']]
                signature = next_block['signature']
                return Block(transactions, next_block['block']['node_pubkey'],
                             next_block['block']['timestamp'], next_block['block']['voters'], signature)
    ...
    def requeue_transactions(self, invalid_block):
        ...
        if self.bigchain.me == invalid_block.node_pubkey and isHandled == False:
            logger.info('Rewriting %s transactions from invalid block %s', len(invalid_block.transactions),
                        invalid_block.id)
            if monitor is not None:
    ...
    def check_local_mem(self, invalid_block):
        ...
        if not isHandled:
            ...
            if nodeIndex < myIndex:
                endtime = time() + (myIndex - nodeIndex) * config['argument_config'][
                    'election_pipeline.wait_time']  # 每个节点需要n秒钟处理时间，待定为10s
            else:
                endtime = time() + (len(self.bigchain.nodelist) - nodeIndex + myIndex) * config['argument_config'][
                    'election_pipeline.wait_time']
